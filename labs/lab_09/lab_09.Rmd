...
april_data %>%
  ...
  labs (
    title = "West Virginia April Loans By Date",
    x = "Total Loans",
    y = "Date Approved",
    caption = "SBA PPP loan database"
  ) +
  geom_point(
    aes(x=date_approved, y=total_loans)) +
   scale_x_date(date_breaks = "1 day", date_labels = "%m-%d-%Y") +
  geom_text(
    aes(x=date_approved, y=total_loans,label= total_loans),nudge_x = -1, nudge_y = 0, check_overlap = 1)
...
```
**April 28, 2020 had the most loans approved in West Virginia.** 
...
state_pop_acs <- get_acs(geography = "state",
                         variables = c(population = "B01001_001"))

#organizing
state_ppp_acs <- state_pop_acs %>%
  left_join(state_ppp_all, by=c("NAME" = "state"))
