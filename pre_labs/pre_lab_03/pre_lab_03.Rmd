...
**The amount field is a double class, the name field is a character and the id column is a double class**
```{r}
...
**Where the franchise name is NA there are 192959 loans. Subway had 275 loans.**:
...
**There are 135073 loans where the forgiveness date is NA. it is the same as the number for forgiveness amount.**:
...
**There are 135073 loans where the forgiveness date and amount are both NA. This amount is the same as when the two were run separately.**:
```{r}
ppp_maryland_loans %>%
...
**There are zero loans in the data for October 2020.**
```{r}
ppp_maryland_loans %>%
...
**The largest loan is for 10000000 and the smallest loan is for 6. This is suspicious because six seems suprisingly small for a minimum value in this context.**
```{r}
...
**There are six rows in this dataset.**
...
**The amount field is a character data type.**
```{r}
...
**Some city values are in all-caps while others are lowercase, some zip codes include an extra four digits, and some addresses are capitalized.**
...
**The clean function standardizes the number of digits in the 1_id column.**
...
**The error says "invalid 'type' (character) of argument". I think this means there is an issue with the sum function.**
...
**Amount has changed to a couble class function.**
...
**There are two dupes of Stuart Jones LLC.**
...
**The new dataframe has five rows.**
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Two mispellings of Arnold remain.**
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**One mispelling of Arnold remains.**
...
**No mispellings of Arnold remain.**
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
