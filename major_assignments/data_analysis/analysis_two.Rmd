...
senate_agreements <- read_rds("data/senate_agreements.rds")%>%
 ...
 "Other (Sole Proprietorship, now inactive)"
    ...
    mutate(type = case_when(
      type == "Continuing participation in an employee benefit plan" ~ "former employer payments",
      type == "Continuing participation in an employee benefit plan" ~ "leave of absence",
      type == "Other (Promissory note)" ~ "promisory note",
      type == "Other (Sole Proprietorship, now inactive" ~ "sole proprietorship",
      TRUE ~ type
    ))
# Continuation of payments by former employer
senate_agreements %>%
  filter(type %in% "Continuing participation in an employee benefit plan") 

  
agreement_types <- senate_agreements %>%
  distinct(type)

# Leave of absence
senate_agreements %>%
  filter(type == "Leave of Absence")
senate_agreements

# Other (Promissory note)
senate_agreements %>%
  filter(type == "Other (Promissory note)")

# Other (Sole Proprietorship, now inactive)

senate_agreements %>%
  filter(type == "Other (Sole Proprietorship, now inactive")
